...
function useOpenState(openFromParent, isInRange) {
  ...
  return [computedOpenState, setOwnOpenState] as const;
}
...
const Element = React.memo(
  function Element({
    ...
    let valueOutput: any = null;
    let content: any = null;
    let prefix: any = null;
    let suffix: any = null;
    let showToggler: boolean = false;
    ...
    if (value && typeof value === "object") {
      ...
      if (typeof value.length === "number") {
        if (value.length > 0 && isOpen) {
          ...
          suffix = "]";
          const node = value;
          let elements: any = Array.from(treeAdapter.walkNode(value))
            .filter(({ key }: any) => key !== "length")
            .map(({ key, value, computed }: any) =>
              renderChild(
                key,
                value,
                node,
...
const FunctionElement = React.memo(function FunctionElement(props: any) {
